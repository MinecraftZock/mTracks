...
import info.mx.tracks.ops.ImportHelper.getCountryFromLatLng
...
internal class OpTracksMapLoadOperation : AbstractOpTracksMapLoadOperation(), KoinComponent {
        ...
        for (fileRecord in files) {
            ...
            for (opt in options) {
                ...
                for (i in bodyList.indices) {
                    ...
                    if (tmpline.lowercase(Locale.getDefault()).contains("a sx-style track") || tmpline.lowercase(Locale.getDefault())
                            .contains("a little sx-style track") || tmpline.lowercase(Locale.getDefault())
                            .contains("SX") || tmpline.lowercase(Locale.getDefault())
                            .contains("supercross")
                    ) {
                        sx = true
                    ...
                    if (tmpline.lowercase(Locale.getDefault()).contains("only for races") || tmpline.lowercase(Locale.getDefault())
                            .contains("race track") ||
                    ...
                    if (!startDatum(tmpline) && (tmpline.lowercase(Locale.getDefault())
                            .contains("club drivers only") ||
                ...
                for (i in itemsFilter.indices) {
                    if (i < itemsFilter.size - 1 && startDatum(itemsFilter[i]) && isZeitZeile(
                            itemsFilter[i + 1]
                        )
                    ) {
                ...
                for (www in wwwList) {
                    if (www != "" && !www.contains("offroadxfilms") && www != "http:/") {
                        var zw = www
                        if (zw.endsWith("com") && zw.substring(
                                zw.lastIndexOf("com") - 1,
                                zw.lastIndexOf("com")
                            ) != "."
                        ) {
                            zw = (zw + "y").replace(
...
$zw""".trim { it <= ' ' }
    ...
    private fun isPhoneNumber(zeile: String): Boolean {
        ...
        return (numOnly.length > 6 && temp.length > 7 && temp.substring(1, 2) != "." && temp.substring(2, 3) != "." &&
                temp.substring(2, 3) != ":" && temp.substring(0, 1).matches(regularExpression.toRegex()))
    }
    ...
    private fun isZeitZeile(zeile: String): Boolean {
        ...
        return if (temp.isEmpty() || !(temp.indexOf("-") in 1..5)) {
            ...
            val tok1 = temp.substring(0, temp.indexOf("-")).replace(":", ".").replace("(", "")
            ...
            t1 > 8 && tok2 == "" || t1 > 6 && t1 < 20 && t2 > 9 && t2 < 24 || tok2.substring(
                0,
